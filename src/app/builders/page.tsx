...
async function getBuilders() {
  return db.user.findMany({
    ...
    include: { _count: { select: { projects: true, updates: true } } },
  })
}
...
export default async function BuildersPage() {
  ...
  return (
    <div className="mx-auto max-w-6xl px-6 py-12">
      <div className="mb-8">
        <h1 className="font-display text-3xl font-black text-brand-navy">Builders</h1>
      ...
      {builders.length > 0 ? (
        ...
        <div className="flex flex-col items-center justify-center rounded-xl border border-dashed border-brand-indigo/15 py-16 text-center">
          <p className="text-brand-navy/45">No builders yet. Sign in to join the community.</p>
        </div>
      )}
    </div>
  )
