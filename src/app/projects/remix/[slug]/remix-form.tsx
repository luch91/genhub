...
export function RemixForm({ remixedFromId, defaultTitle, defaultTags }: Props) {
  ...
  const [selectedTags, setSelectedTags] = useState<string[]>(
    defaultTags.filter((t) => PREDEFINED_TAGS.includes(t as (typeof PREDEFINED_TAGS)[number]))
  )
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6">
      ...
      <Field label="Project title" error={errors.title}>
        <input name="title" className="input" defaultValue={defaultTitle} />
      </Field>

      <Field label="Tagline" hint="One sentence — what does your remix do differently?" error={errors.tagline}>
        <input name="tagline" className="input" placeholder="A remix that adds X to Y" />
      </Field>

      <Field label="Description" error={errors.description}>
        <textarea name="description" className="textarea" rows={5} placeholder="Describe your remix in detail..." />
      </Field>

      <Field
        label="Why only GenLayer?"
        hint="What makes this remix only possible on GenLayer?"
        error={errors.genlayerAngle}
      >
        <textarea name="genlayerAngle" className="textarea" rows={4} placeholder="This remix couldn't exist on any other platform because..." />
      </Field>
